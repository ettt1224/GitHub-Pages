...
<head>
    ...
    <title>{{ $name }} | 2025 響應式儀表板履歷</title>
    ...
    <style>
        body { background-color: #020617; color: #f8fafc; font-family: 'Space Grotesk', sans-serif; }
        
        /* 桌機版鎖定高度，手機版允許捲動 */
        @media (min-width: 768px) {
            .full-screen-lock { height: 100vh; overflow: hidden; }
        }
...
<body class="antialiased full-screen-lock flex flex-col p-4 md:p-10 overflow-y-auto md:overflow-hidden">

    <div class="glow-bg"></div>

    <!-- Main Container: 100% Height on Desktop, Auto on Mobile -->
    <div class="flex-1 flex flex-col md:flex-row gap-6 md:gap-8 max-w-7xl mx-auto w-full overflow-y-auto md:overflow-hidden">
        
        <!-- 左側欄位 (100% on mobile, 50% on desktop) -->
        <div class="w-full md:w-1/2 flex flex-col gap-6">
            ...
            <header class="flex-none text-center md:text-left">
                ...
                <p class="text-base md:text-lg text-slate-400 font-medium uppercase tracking-[0.2em]">{{ $title }}</p>
            </header>
            ...
            <section class="flex-1 glass p-6 md:p-8 rounded-[2rem] flex flex-col gap-4">
                ...
                <div class="grid grid-cols-1 gap-4 md:flex-1 md:flex md:flex-col md:justify-around">
                    ...
                    <div class="flex items-center gap-4 group">
                        ...
                        <div class="flex-1">
                            <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{{ $group['category'] }}</p>
                            <p class="text-lg font-black text-white group-hover:text-blue-400 transition">{{ $group['items'] }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <!-- 右側欄位 (100% on mobile, 50% on desktop) -->
        <div class="w-full md:w-1/2 flex flex-col gap-6">
            <!-- 聯絡方式 -->
            <section class="flex-none glass p-6 md:p-8 rounded-[2rem] bg-gradient-to-br from-purple-500/5 to-transparent relative overflow-hidden">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <p class="text-sm font-bold truncate">{{ $contact['email'] }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                ...
                <a href="mailto:{{ $contact['email'] }}" class="mt-4 block w-full py-3 bg-white text-dark text-center font-black rounded-xl hover:bg-blue-400 transition text-sm">
                    立即與我聯繫
                </a>
            </section>
            ...
            <section class="flex-1 glass p-6 md:p-8 rounded-[2rem] flex flex-col min-h-[300px] md:min-h-0">
                <div class="flex justify-between items-center mb-4 border-b border-purple-500/20 pb-2">
                    ...
                    <a href="{{ $contact['github'] }}" class="text-[9px] font-bold text-blue-400 hover:text-white transition uppercase">View All ↗</a>
                </div>
                ...
                <div class="flex-1 flex flex-col gap-3 overflow-y-auto md:overflow-hidden">
                    ...
                    <div class="flex-none md:flex-1 glass p-4 rounded-2xl border border-white/5 hover:border-blue-500/30 transition flex flex-col justify-center group relative overflow-hidden project-item">
                        <h3 class="text-base font-bold group-hover:text-blue-400 transition mb-1">{{ $project['name'] }}</h3>
                        <p class="text-slate-400 text-[11px] line-clamp-2 leading-snug">{{ $project['desc'] }}</p>
                    </div>
                    @endforeach
                </div>
            </section>
